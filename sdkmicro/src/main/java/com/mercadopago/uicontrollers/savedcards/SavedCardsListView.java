...
public class SavedCardsListView implements CustomViewController {
    ...
    public SavedCardsListView(Context context, List<Card> cards, String footerText, OnSelectedCallback<Card> onSelectedCallback, Drawable selectionImage) {
        mContext = context;
        mFooterText = footerText;
        mCards = cards;
        mOnselectedCallback = onSelectedCallback;
        mSelectionDrawable = selectionImage;
    }
    ...
    public void drawInParent(ViewGroup parent) {
        ...
        // Load cards
        mSavedCardsRecyclerView.setAdapter(new CustomerCardsAdapter(mContext, mCards, mOnselectedCallback, mSelectionDrawable));
